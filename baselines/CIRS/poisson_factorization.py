# -*- coding: utf-8 -*-
"""Poisson_Factorization.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Ru0MKG6Xd1GCMJzgPnOxuyuqAXZUjCKd
"""
...
def _inner(beta, theta, rows, cols):
    ...
    for i in range(0,n_ratings):
        ...
        for j in range(0,n_components):
            data[i] += beta[rows[i]][j]*theta[cols[i]][j]
    return data
...
def load_data(csv_file, shape):
        ...
        rows, cols, vals = np.array(tp['userId']), np.array(tp['productId']), np.array(tp['rating'])
...
train = load_data("/content/drive/MyDrive/Copy of Epinions/Ciao/ratings_data.csv",shape=(7375, 105114))
#val = load_data("/content/drive/MyDrive/Copy of Epinions/Ciao/user_ratings_val_new.csv",shape=(7375, 105114))
#test = load_data("/content/drive/MyDrive/Copy of Epinions/Ciao/user_ratings_test_new.csv",shape=(7375, 105114))

print("Train, Val and Test going to be converted to implicit...")
train_data_imp = exp_to_imp(train,0.5)
#vad_data_imp = exp_to_imp(val, 0.5)
...
for i, dim in enumerate(dims):
    print("dim", dim)
    pf = PoissonMF(n_components=dim, random_state=98765, verbose=True, a=1, b=1, c=1, d=1)
    print(train.data.shape)
    print(row_tr.shape)
    pf.fit(train_data_coo, train_data_coo.row, train_data_coo.col)
    U, V = pf.Eb, pf.Et.T
    np.savetxt('/content/drive/MyDrive/Copy of Epinions/Ciao/data/cause_pmf_k' + str(dim) + '_U.csv', U)
    np.savetxt('/content/drive/MyDrive/Copy of Epinions/Ciao/data/cause_pmf_k' + str(dim) + '_V.csv', V)